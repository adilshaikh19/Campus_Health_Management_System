...
def register(request):
    ...
    # If the user wants to submit his/her information
        ...
        if userPassword == userConfirmPassword:
            ...
            patient = Patient(name=name, rollNumber=userRollNo, email=userEmail, passwordHash=passwordHash,
                              address=userAddress, contactNumber=userContactNo, emailHash=emailHash)
            patient.save()
            ...
            # Editing response headers so as to ignore cached versions of pages
            response = render(
                request, "HealthCentre/registrationPortal.html", context)
            return responseHeadersModifier(response)
        ...
        try:
            ...
            if request.session['isLoggedIn'] and request.session['isDoctor']:
                ...
                doctor = Doctor.objects.get(
                    emailHash=request.session['userEmail'])
                records = doctor.doctorRecords.all()

                # Getting the count of the new prescriptions pending
                numberNewPendingPrescriptions = doctor.doctorRecords.aggregate(newPendingPrescriptions=Count(
                    'pk', filter=(Q(isNew=True) & Q(isCompleted=False))))['newPendingPrescriptions']
                ...
                # Editing response headers so as to ignore cached versions of pages
                response = render(
                    request, "HealthCentre/userDoctorProfilePortal.html", context)
                return responseHeadersModifier(response)
            ...
            # If the user is already logged in inside of his sessions, and is a patient, then no authentication required
                ...
                # Getting the count of the new prescriptions pending
                numberNewPrescriptions = patient.patientRecords.aggregate(newCompletedPrescriptions=Count(
                    'pk', filter=(Q(isNew=True) & Q(isCompleted=True))))['newCompletedPrescriptions']
    ...
    # If the request method is post
        ...
        # Otherwise trying if a doctor exists
            ...
            # If no such doctor or patient exists
                ...
                # Editing response headers so as to ignore cached versions of pages
                response = render(
                    request, "HealthCentre/loginPortal.html", context)
                return responseHeadersModifier(response)
...
def emergency(request):
    ...
    # If the request method is post and the user is submitting information
        ...
        if emergencyLocation != "":
            ...
            print(
                "------------------------------------------------------------------------")
            print("\n\nEMERGENCY !! AMBULANCE REQUIRED AT " +
                  emergencyLocation + " !!\n\n")
            print(
                "------------------------------------------------------------------------")
            ...
            # Editing response headers so as to ignore cached versions of pages
            response = render(
                request, "HealthCentre/emergencyPortal.html", context)
            return responseHeadersModifier(response)
    ...
    if request.method == "GET":
        ...
        if request.session['isLoggedIn']:
            ...
            if request.session['isDoctor']:
                ...
                # Editing response headers so as to ignore cached versions of pages
                response = render(
                    request, "HealthCentre/prescriptionPortal.html", context)
                return responseHeadersModifier(response)
        ...
        if request.session['isLoggedIn']:
            ...
            if request.session['isDoctor']:
                ...
                prescription = Prescription.objects.get(
                    pk=request.POST['prescriptionID'])
                ...
                records = Doctor.objects.get(
                    emailHash=request.session['userEmail']).doctorRecords.all()
            ...
            # Else if the patient is submitting prescription request
                ...
                prescription = Prescription(doctor=doctor, patient=Patient.objects.get(
                    emailHash=request.session['userEmail']), symptoms=symptoms)
                prescription.save()
...
def bookappointment(request):
    ...
    if request.method == 'POST':
        ...
        # Get user name and email from session variables
        user_name = request.session.get('Name', 'Anonymous User')
        user_email = request.session.get('userEmail', 'anonymous@example.com')

        # Create and save the appointment
        doctor = Doctor.objects.get(id=doctor_id)
        appointment = Appointment(
            appointment_date=appointment_date, doctor=doctor,timeslot=timeslot, reason=reason, user_name=user_name,
    ...
    return render(request, 'HealthCentre/bookappointment.html', {'doctors': doctors})
...
def my_bookings(request):
    ...
    if request.session.get('isLoggedIn', False):
        user_email = request.session.get('userEmail', 'anonymous@example.com')
        appointments = Appointment.objects.filter(user_email=user_email).order_by("-appointment_date")
        return render(request, 'HealthCentre/my_bookings.html', {'appointments': appointments})
    else:
        return redirect('login')
